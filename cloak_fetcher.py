...
_last_search_time = 0.0  # 上次搜索时间戳，用于控制搜索间隔
...
def _enforce_search_interval(min_seconds=5.0, max_seconds=10.0):
    """确保两次搜索之间有合理间隔（仿人行为）。"""
    global _last_search_time
    if _last_search_time > 0:
        elapsed = time.time() - _last_search_time
        target = random.gauss((min_seconds + max_seconds) / 2, 2.0)
...
def _get_browser(humanize=False):
    ...
    Args:
        humanize: True 时启用仿人浏览模式（UA/分辨率/语言随机化）
    """
...
def search_google(query, max_results=5, max_retries=3):
    """用 CloakBrowser 执行 Google 搜索，带 429 自动轮换。
